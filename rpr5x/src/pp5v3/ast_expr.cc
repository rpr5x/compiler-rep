...
void ArithmeticExpr::Emit()
{
  ...
  else //Normal binary arithmetic
  {
     ...
     else if(var1)
     {
       ...
       IntConstant *ic = dynamic_cast<IntConstant *>(right);
...
void RelationalExpr::Emit()
{
  ...
  ArrayAccess * a2 = dynamic_cast<ArrayAccess *>(right);
...
void AssignExpr::Emit()
{
      ...
      if(classParent)
      {
         ...
         {         
           ...
           { 
             ...
             //If this is a derived class, increment the offset counter by 4 for each instance variable
             //in the base class
             if(classParent->GetExtends())
             {
               NamedType * nt = classParent->GetExtends();
               Decl * d = FindDecl(nt->GetId());
  ...
  else if(left2 && right4)//Case 3: The left side is a variable and the right side is a NewArrayExpr
  {
     ...
     if(v1)
     {
       ...
       //Check for instance variable assignment.  This needs to be done sightly differenly than a normal variable assignment
       Node * parent = v1->GetParent();
       ClassDecl * classParent = dynamic_cast<ClassDecl *>(parent);
       Program * prog = dynamic_cast<Program *>(parent);
       ...
       if(classParent)
       {
          ...
          {    
            ...
            if(var && strcmp(var->GetId()->GetName(), v1->GetId()->GetName()) == 0)
            {
               ...
               FnDecl::numBytes-=4; //We don't want to count instance variables in our calculation
             
               
               
            }
            /*else if(var && strcmp(var->GetDeclaredType()->GetName(), "int") == 0) //This may need to be changed
            {
              int offset2 = 4 *(i+1);
              Location * zeroLoc = codeGen->GenLoadConstant(0);
              codeGen->GenStore(codeGen->ThisPtr, zeroLoc, offset2); 
  ...
  else if(left2 && right7) //Case 6:  The left side is a variable and the right side is an arithmetic expression
  {
     ...
     {
       Location * varLoc;
       Location * rightLoc = right7->GetLocationNode();
       ...
       if(classParent)
       {
          ...
          {    
            ...
            if(var && strcmp(var->GetId()->GetName(), v1->GetId()->GetName()) == 0)
            {
               int offset = 4 *(i+1);     
               codeGen->GenStore(codeGen->ThisPtr, rightLoc, offset); 
               FnDecl::numBytes-=4;  //Don't count instance variables
            }
          }
...
void Call::Emit()
{
  ...
  for(int i = actuals->NumElements() - 1; i >= 0; i--)
  {
    ...
    NullConstant *nc = dynamic_cast<NullConstant *>(actuals->Nth(i));  
    ...
    if(f)
    {
      ...
      Program * prog = dynamic_cast<Program *>(node);

      while(!classDecl && !prog)
      {      
        node = node->GetParent();
      ...
      if(!d && classDecl)
      {
        ...
        for(int i = 0; i < members->NumElements(); i++)
        {
          ...
          if(fnMember)
          { 
            List<VarDecl *> *formals = fnMember->GetFormals();
            codeGen->GenPushParam(formals->Nth(0)->GetLocationNode());
            tempLoc = formals->Nth(1)->GetLocationNode();
            break;
          }
        }  
      }
    ...
    {
      tempLoc = codeGen->GenLoadConstant(0);
    }
    else if(a)
    {
  ...
  if(classParent && fnLoc2) 
  {
   ...
   if(classParent->GetExtends()) 
     resultLoc = codeGen->GenACall(fnPtr,hasReturn);    
   else 
     resultLoc = codeGen->GenACall(fnLoc2, hasReturn);
     if(hasReturn == true)
       FnDecl::numBytes+=4;
  ...
  if(base)
  {
    ...
    {    
      ...
      {        
        ...
        {
          const char * baseName = baseVar->GetId()->GetName();
          Location * thisLoc;
          if(strcmp(baseName, "this") == 0)
             thisLoc = new Location(fpRelative, 4, baseVar->GetLocationNode()->GetName());     
          else
          {
          ...
          if(baseNameType)
          {
            ...
            Identifier * ntName = baseNameType->GetId();
            Decl * classDecl = FindDecl(ntName);
            ClassDecl * classDecl2 = dynamic_cast<ClassDecl *>(classDecl);
            ...
            {         
              ...
              { 
                Decl * member = members->Nth(i);
                FnDecl * fnMember = dynamic_cast<FnDecl *>(member);
                if(fnMember && strcmp(field->GetName(), fnMember->GetId()->GetName()) == 0)
                {
                  Type * fnType = fnMember->GetType();
                 
                  if(strcmp(fnType->GetName(), "void") == 0)
                   codeGen->GenACall(fnLoc, false);
                  else
                  {
                    fnMember->fnDeclLoc = codeGen->GenACall(fnLoc, true);
                    FnDecl::numBytes+=4;
                  }
                }
              }
  ...
  //Check for class methods here since fn will be null for them (since they are not kept in the symbol table)
  /*if(base)
  { 
    FieldAccess * baseField = dynamic_cast<FieldAccess *>(base);
    if(baseField)
        ...
        if(nt)
        {
          ...
          {         
            ...
            { 
              Decl * member = members->Nth(i);
              FnDecl * fnMember = dynamic_cast<FnDecl *>(member);
              if(fnMember && strcmp(field->GetId()->GetName(), fnMember->GetId()->GetName()) == 0)
