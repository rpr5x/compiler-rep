...
void VarDecl::Emit()
{
  ...
  /*if(classParent) //Handles instance variables of a class
  {
...
void ClassDecl::Emit()
{
  ...
  //Append all function labels located in the base class's vtable into the vtable for the derived class
  if(this->extends)
  {
    NamedType *superClass = this->extends;
    Decl * superClassDecl = FindDecl(superClass->GetId());
    ClassDecl * superClass2 = dynamic_cast<ClassDecl *>(superClassDecl);
    if(superClass2)
    {
      List<const char *> *superLabels = superClass2->GetFnLabels();
      
      for(int i = 0; i < superLabels->NumElements(); i++)
      {
        const char * superLabel = superLabels->Nth(i); 
        this->fnLabels->Append(superLabel);
      }
    }
